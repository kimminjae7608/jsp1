...
public class MybatisDao {
	...
	//2. SqlSession 객체 생성하여 sql 실행하기.
		...
		list = mapper.selectList("selectAll");	//"selectAll" mapper 파일의 id값과 일치해야 합니다.
						// ㄴ 이메소드는 리턴타입이 List<Object>, select 조회결과가 n개 row일때
		mapper.close();
		return list;
	}
	...
	public int insert(Customer cus) {
		...
		mapper.commit();	//1. auto commit을 true로 설정	2. commit() 직접 설정
		mapper.close();
		return cus.getIdx();
	}
